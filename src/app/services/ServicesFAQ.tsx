...
const SVC_FAQS = [
  {
    q: "GEOとSEOは何が違うのですか？",
    a: "GEOはSEOの「次バージョン」ではなく、評価軸そのものが異なる別領域です。SEOが検索結果ページの上位表示を目指すのに対し、GEOはChatGPT・Gemini・Perplexityなど生成AIの回答文そのものに引用される状態を目指します。最適化の対象・ゴール・評価単位がすべて異なるため、既存のSEO手法をそのまま流用しても効果は出ません。両者は対立ではなく補完関係です。",
  },
  {
    q: "他社のGEOサービスと何が違うのですか？",
    a: "GEOに好まれる質問を分析してコンテンツ対策を行う会社は増えていますが、Ascentは「どんな質問がされているのか」をデータで見て、質問の意図に合わせてコンテンツ対策を行える点が大きな差別化です。競合他社の多くはGAP分析・コンテンツ対策・モニタリングのみの対応ですが、Ascentはその前段となる質問クラスター分析（Question Cluster Mining）から一貫して対応しています。",
  },
  {
